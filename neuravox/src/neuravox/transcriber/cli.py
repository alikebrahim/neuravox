...
from ..shared.config import UnifiedConfig, ProjectConfig
from .engine import AudioTranscriber
...
async def run_transcription(project: ProjectConfig):
    ...
    try:
        ...
        with Progress(
            ...
            for i, audio_file in enumerate(project.audio_files):
                ...
                try:
                    ...
                    if not has_success:
                        config.save_project_config(project)
                        has_success = True
        ...
        if has_success:
            config.add_recent_project(project.name)
